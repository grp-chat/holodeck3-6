...
function loadObtainedPowersToModal(powersArr, teamNum) {
    
    modalHeader.innerHTML = `${nickname} - ${teamDetails[teamNum].art}: ${teamDetails[teamNum].desc} <br>`;
    modalHeader.innerHTML += `Abilities: ${teamDetails[teamNum].abilities}`;
    modalBody.innerHTML = "";
...
sock.on('updatePowerArray', data => {
    ...
    loadObtainedPowersToModal(data.obtainedPowers, data.getTeam);
});
